...
def get_geometry(lon, lat, size):
    ...
    geo_json_geometry = {
        "type": "Polygon",
        "coordinates": [[[lon - size / 2, lat - size / 2],
...
def get_item_ids(client,
                 geo_json_geometry,
                 start="2017-11-01T00:00:00.000Z",
                 stop="2017-11-15T00:00:00.000Z",
                 limit=10):
    ...
    def get_timeseries(self):
        ...
        for img in images:
            with rasterio.open(img) as data:
                ...
                ratios.append(ratio)
                time = datetime.strptime(data.tags()['TIFFTAG_DATETIME'],
                                         "%Y:%m:%d %H:%M:%S")
                times.append(time)
        return times, ratios
